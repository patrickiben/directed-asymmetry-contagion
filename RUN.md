...
| Citation integrity (deterministic) | `python3 tools/check_citations.py <manuscript>.tex` | Every reference resolves on CrossRef / OpenAlex / arXiv; title-containment match + year check; flags fabrication, identifier-hijacking (a DOI → a *different* paper), dead DOIs, retractions; plus a `cited == listed` parity check. No LLM. |
| Citation metadata (adversarial) | `python3 tools/check_citations.py <ms>.tex --dump-entries` then run the Workflow `tools/citation_metadata.workflow.js` with those entries as `args` | Resolve→refute each reference's *fine* metadata (volume / issue / pages / venue / author set) that a containment score cannot see. **Run both:** the deterministic gate catches existence / hijack / dead-DOI / year; this catches subtler metadata errors (the false-floor "right title, wrong volume" case). |
